...
--Bang 1
create table ThongTinMayTinh(MaMT char(10) primary key, tenmt varchar(50), MaNCC char(10) foreign key (MaNCC) references tblNhaCC(MaNCC) on delete cascade on update cascade
...
--Bang 5
create table Hoadonnhap(MaHDN char(10) primary key,MaNV char(10) foreign key (MaNV) references tblNhanvien(MaNV) on delete cascade on update cascade
,MaMT char(10) foreign key (MaMT) references tblThongtinMT(MaMT) on delete cascade on update cascade
,MaNCC char(10) foreign key (MaNCC) references tblNhaCC(MaNCC) on delete cascade on update cascade
,Soluong int,Ngayban date,Diachi varchar(50),sdt int, Dongia float,Tongtien float)
--Bang 6
create table Hoadonban(MaHDB char(10) primary key,MaNV char(10) foreign key (MaNV)references tblNhanvien(MaNV) on delete cascade on update cascade
